...
import com.stage1.Adapters.VideoAdapter;
...
*/
    ...
    public VideoFragment(Context activity) {
...
//        medialist.add(medialist.get(0));
        List<GetAllMediaDetail.Datum> filtered = new ArrayList<>();
//        filtered.add(medialist.get(0));
        ...
        galaryAdapter = new VideoAdapter(filtered, new VideoAdapter.OnItemClickListener() {
            @Override
...
//                img.setImageResource(v.getimage);
                ...
                if (medialist.get(item-1).getFileType() == 2)/* {
...
//                    MediaController mediacontroller  = new android.widget.MediaController(getContext());
                    ...
                    pw.dismiss();
                    startActivity(new Intent(getContext(), VideoViewActivity.class).putExtra("path", medialist.get(item-1).getFilePath()));
                } else {
