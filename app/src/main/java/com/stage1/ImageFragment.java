...
import com.stage1.Adapters.SlidingImage_Adapter;
...
public class ImageFragment extends Fragment {
    ...
    public ImageFragment(Context activity) {
...
*/
    ...
    private void uploadMedia() {
        String userid = new PrefManager(context).getUser_pref().getString(User_Constant.id, "");
...
//        medialist.add(medialist.get(0));
        final List<GetAllMediaDetail.Datum> filtered = new ArrayList<>();
//        filtered.add(medialist.get(0));
        for (int i=0;i<medialist.size();i++)
        {
            if (medialist.get(i).getFileType()!=2)
            {
        ...
        galaryAdapter = new GalaryAdapter(filtered, new GalaryAdapter.OnItemClickListener() {
            @SuppressLint("NewApi")
            @Override
...
//                View layout = inflater.inflate(R.layout.activity_image_dialog, (ViewGroup) getActivity().findViewById(R.id.img_dialog_constraint));
                View layout = inflater.inflate(R.layout.image_fragment_popup,null);
                // create a 300px width and 470px height PopupWindow
                Drawable dim = new ColorDrawable(Color.BLACK);
                ImageView img;
//                img = layout.findViewById(R.id.img_dialog);
                final VideoView videoView;
//                videoView = layout.findViewById(R.id.video_dialog);
...
//                img.setImageResource(v.getimage);
                ...
                if (filtered.get(item-1).getFileType() == 2)/* {
...
//                    MediaController mediacontroller  = new android.widget.MediaController(getContext());
                    ...
                    pw.dismiss();
                    startActivity(new Intent(getContext(), VideoViewActivity.class).putExtra("path", filtered.get(item).getFilePath()));
//                } else {
...
//                    Glide.with(img.getContext()).load(new PrefManager(context).getpath()+
//                            filtered.get(item-1).getFilePath()).into(img);
//                }
                }else {
                    mPager = (ViewPager) layout.findViewById(R.id.pager_image);


                    mPager.setAdapter(new SlidingImage_Adapter(context,medialist));
            ...
            public void onItemRemove(final int item) {
                AlertDialog.Builder builder= new AlertDialog.Builder(getContext()).setMessage("Are you sure want to delete?").setNegativeButton("Ok", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        ApiInterface apiInterface = ApiClient.getClient().create(ApiInterface.class);
                        Call<JsonObject> removeMedia = apiInterface.removeMedia(filtered.get(item).getId());
                        if (pDialog!=null)
//                        pDialog.show();
//                        else
                            createDialog();
